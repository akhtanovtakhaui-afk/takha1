...
LANES = [200, 300, 400] #это позиции полос дороги (lane positions)
SPEED_BASE = 5#базовая скорость объектов

def load_image(name, width, height):
    path = os.path.join('assets', 'images', name)
    try:
        image = pygame.image.load(path).convert_alpha()#Загрузка картинки.прозрачность
        return pygame.transform.scale(image, (width, height))
    except FileNotFoundError:
        surf = pygame.Surface((width, height))#создаём заглушку
        surf.fill((255, 0, 255)) 
        return surf

class Player(pygame.sprite.Sprite):#это класс для игровых объектов
    ...
    def update(self):
        keys = pygame.key.get_pressed()# возвращает состояние всех клавиш прямо сейчас
...
class Obstacle(pygame.sprite.Sprite):#препяствие
    def __init__(self):
        super().__init__()
...
class PowerUp(pygame.sprite.Sprite):#бонусы
    def __init__(self):
        super().__init__()
