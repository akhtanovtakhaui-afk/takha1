...
from persistence import load_settings, save_settings, load_leaderboard, save_score#это файл для сохранения данных
from ui import Button, TextInput#интерфейс
from racer import Player, Enemy, Obstacle, PowerUp

# АВТОМАТИЧЕСКОЕ ИСПРАВЛЕНИЕ ПУТИ
os.chdir(os.path.dirname(os.path.abspath(__file__)))#чтобы программа всегда работала из папки проекта

# Инициализация
pygame.init()
pygame.mixer.init()#ключает систему звука в Pygame
WIDTH, HEIGHT = 600, 600
screen = pygame.display.set_mode((WIDTH, HEIGHT))
pygame.display.set_caption("TSIS 3: Racer")
clock = pygame.time.Clock()
font = pygame.font.Font(None, 36)#создаёт шрифт для текста
...
all_sprites = pygame.sprite.Group()#список игровых объектов
...
def reset_game():
    ...
    all_sprites.empty()#очистить группу
    enemies.empty()
    obstacles.empty()
    powerups.empty()
    ...
    if music_loaded and settings["sound"]:
        pygame.mixer.music.play(-1)#запускает фоновую музыку,play(-1) = играть бесконечно

def draw_hud():#рисует информацию на экране во время игры
    screen.blit(font.render(f"Score: {int(score)}", True, (255,255,255)), (10, 10))# показывает очки
...
# Таймеры появления
SPAWN_ENEMY = pygame.USEREVENT + 1#ты создаёшь свои события (custom events)
SPAWN_OBSTACLE = pygame.USEREVENT + 2#бонустар шыгады
...
while running:
    ...
    for event in events:
        ...
        if state == "MENU":
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_RETURN:
                ...
                reset_game()#начать игру заново
                state = "PLAY"

        elif state == "PLAY":
            if event.type == SPAWN_ENEMY:
                e = Enemy(settings["difficulty"])
                if not pygame.sprite.spritecollideany(e, enemies):#сталкивается ли новый объект e с уже существующими врагами
                    all_sprites.add(e)
                    enemies.add(e)
            if event.type == SPAWN_OBSTACLE:
                o = Obstacle()#Спавн препятствий
    ...
    # --- РЕНДЕРИНГ ЭКРАНОВ ---
    if state == "MENU":# рисует кнопки меню на экране
        ...
        # Коллизии
        if not player.shield_active:#если щит НЕ активен
            if pygame.sprite.spritecollideany(player, enemies) or pygame.sprite.spritecollideany(player, obstacles):#Проверка столкновений
                if settings["sound"] and snd_crash: snd_crash.play()
                if player.crashes_allowed > 0:# есть ли шанс выжить
                    player.crashes_allowed -= 1# уменьшаем жизни
                    player.shield_active = True#даём временный щит
                    player.powerup_timer = pygame.time.get_ticks() + 2000#щит на 2 секунды
                else:
