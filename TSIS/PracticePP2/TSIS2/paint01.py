...
def get_rtri_pts(p1, p2):
    return [p1, (p1[0], p2[1]), p2] #Это функция, которая создаёт 3 точки для прямоугольного треугольника
...
def get_eqtri_pts(p1, p2):
    ...
    by = max(p1[1], p2[1])
    side = bx2 - bx1
    return [((bx1 + bx2) // 2, by - int(side * 0.866)), (bx1, by), (bx2, by)]#равносторонний треугольник 


def get_rhombus_pts(p1, p2):#строит ромб
    x1, y1 = p1
    x2, y2 = p2
    return [((x1 + x2) // 2, y1), (x2, (y1 + y2) // 2), ((x1 + x2) // 2, y2), (x1, (y1 + y2) // 2)]


def flood_fill(surface, pos, fill_color):# заливка
    target_color = surface.get_at(pos)#выбирает цвет басканына карай
...
class PaintApp:
    ...
    def draw_ui(self):
        ...
        for i, col in enumerate(PALETTE):
            r, c = divmod(i, 4)#возвращает два значения сразу
    ...
    def handle_events(self):#обработка событий 
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if self.text_active:#включён ли режим ввода текста
                    if event.key == pygame.K_RETURN:
                        txt_surface = self.text_font.render(self.text_input, True, self.active_color)# превращает текст в картинку 
                    ...
                    if event.key == pygame.K_1:
                        self.brush_size = 2 #это размер кисти
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:#    button == 1 → левая кнопка
                mx, my = event.pos
                if mx < TOOLBAR_W:
                    # Логика клика по тулбару=место, где кнопки
                    if 10 < my < 380:
                        tool_list = [
                            TOOL_PENCIL, TOOL_LINE, TOOL_RECT, TOOL_SQUARE, TOOL_CIRCLE,
                            TOOL_RTRIANGLE, TOOL_EQTRIANGLE, TOOL_RHOMBUS, TOOL_FILL, TOOL_TEXT, TOOL_ERASER
    ...
    def run(self):
        while True:
            self.handle_events()
            self.screen.fill(BLACK)#заливает весь экран чёрным цветом
            ...
            if self.drawing and self.active_tool not in [TOOL_PENCIL, TOOL_ERASER]:
                m_x, m_y = pygame.mouse.get_pos()# возвращает текущую позицию мыши
            ...
            self.draw_ui()
            pygame.display.flip()#обновляет экран (показывает всё, что ты нарисовал)
            self.clock.tick(60)


if __name__ == "__main__":#запусти код ТОЛЬКО если я запустил этот файл напрямую
    PaintApp().run()